...
import type { Compiled, CompiledContract, CompiledError } from "./solidity.js";

function getRecusriveType(type: ParamType, callback: (t: ParamType) => string): string {
    if (type.isArray()) {
        const str = callback(type.arrayChildren);
        if (type.arrayLength >= 0) {
            const types: Array<string> = [ ];
            for (let i = 0; i < type.arrayLength; i++) { types.push(str); }
            return `[ ${ types.join(", ") } ]`;
        }
        return `Array<${ str }>`;
    }
...
function getType(type: ParamType): string {
    if (type.baseType.match(/^bytes([0-9]*)$/)) { return "string"; }
    if (type.baseType.match(/^u?int([0-9]*)$/)) { return "bigint"; }
...
function getLooseType(type: ParamType): string {
    if (type.type.match(/^bytes([0-9]*)$/)) { return "ethers.BytesLike"; }
    if (type.type.match(/^u?int([0-9]*)$/)) { return "ethers.BigNumberish"; }
...
function generateTypeScript(contract: CompiledContract) {
...
function zpad(_v: number, length: number): string {
    let v = String(_v);
...
async function updateDotSol(root: string, config: any, filename: string): Promise<Compiled> {
...
function walk(root: string, path: string, callback: (fn: string) => void) {
...
function readDir(root: string): Array<string> {
    const result: Array<string> = [ ];
    walk(root, ".", (fn) => { result.push(fn); })
    return result;
}

async function updateDotSols(root: string, config: any, filenames: Array<string>): Promise<Array<CompiledError>> {
    const errors: Array<CompiledError> = [ ];
...
(async function() {
    const config = {
        dirs: {
            contracts: "contracts",  // @TODO: globs, like contract/ ** 
    ...
    const watcher = fs.watch(join(root, config.dirs.contracts), { recursive: true }, async (eventType, filename) => {
        ...
        const filenames: Set<string> = new Set([ filename ]);
        for (const e of errors) { filenames.add(e.filename); }
